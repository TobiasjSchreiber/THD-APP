...
async function loadSchedule() {
  ...
  const iCalLink = 'https://thabella.th-deg.de/thabella/opn/event/calendarStudentSubscribe?group=' + currentStudyGroup;

  // WICHTIG: Die URL codieren, sonst schneidet der Proxy alles nach dem ? oder & ab
  const proxyUrl = 'https://thd-app-backend.onrender.com/api/proxy?url=' + encodeURIComponent(iCalLink);
  ...
  if (weatherWidget) {
    ...
    function handleWeatherSwipe() {
      ...
      if (touchEndX < touchStartX - threshold) {
        ...
        currentForecastIndex = (currentForecastIndex + 1) % forecasts.length;
        updateWeatherWidget();
      } else if (touchEndX > touchStartX + threshold) {
        // Swiped right (previous day)
        currentForecastIndex = (currentForecastIndex - 1 + forecasts.length) % forecasts.length;
        updateWeatherWidget();
      }
    }
